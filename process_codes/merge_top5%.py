...
OUT_PATH = BASE_DIR / "data" / "all-top5.dta"
...
def load_and_merge_data(base_dir: Path, year: int, field: str) -> pd.DataFrame:
    ...
    delay_path   = base_dir / "results" / "top5" / f"{year}{field}_focal_top5_delay_years.csv"
    ...
    ij_path = base_dir / "results" / "top5" / f"{year}{field}_A_C_foreign_ratio_cumulative.csv"
    ...
    try:
        ...
        delay_df.rename(columns={'delay_years_to_top5': 'delay_year'}, inplace=True)
    except FileNotFoundError:
    ...
    df.rename(columns={"author_count":"team_size"},inplace=True)
    ...
    df.rename(columns={"n_phrases":"phrase_count"},inplace=True)
    df.rename(columns={"is_international":"inter_coauthorship"},inplace=True)
    df.rename(columns={"quartile_avg_round_15y_bin_Q12":"journal_tier"},inplace=True)
    df.rename(columns={"quartile_avg_round_15y_bin_Q1":"journal_tier_rc"},inplace=True)
...
merged_all.to_csv(BASE_DIR / "data" / "all-top5.csv", index=False)